...
import { getSurahMeta, getSurahVerses, juzOf, loadBundle } from '@/lib/data';
...
export default function Reading() {
    ...
    (from: number, skipBismillah = false) => {
      ...
      if (from === 1 && !skipBismillah && n !== 1 && n !== 9) {
        ...
        surahHandle.current = playVerse(
          ...
          1,
          () => {
            if (surahGen.current === gen) step(1);
          },
          (pct) => {
            if (surahGen.current === gen) setSurahProgress(pct);
          },
  ...
  const meta = surah ? `${surah.ayahs} verses · Juz ${juzOf(n, 1)}` : undefined;
