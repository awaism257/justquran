...
import { wqDisplay, wqInkOffset, loadBundle, getSurahVerses } from '@/lib/data';
...
export default function FolioPage({ verses, night, activeV = null, fatiha = false }: FolioPageProps) {
  ...
  const startPress = (i: number) => {
    cancelPress();
    pressTimer.current = setTimeout(() => setOpenIdx(i), 450);
  };
  const cancelPress = () => {
      ...
      <div className="folio-inner">
        <p className="folio-ar">
          {verses.map((v, i) => {
            ...
            const runCls = activeV === v.v ? 'verse-run reciting' : 'verse-run';
            const press = {
